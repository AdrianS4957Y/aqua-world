...
class Fish{
	...
	Fish(SendPort sender, Aquarium aquarium){
		this._max_life = Random().nextInt(aquarium.timeList["fishLife"] * aquarium.maxFishes ~/ (aquarium.fishList.length != 0 ? aquarium.fishList.length : 1));
		this.sender = sender;
	...
	addToAquarium(Aquarium aquarium, { mute = false }){
		...
		if(!mute) print("Рыбка ${_firstName} ${_lastName} добавлена в аквариум?! Время жизни: ${_max_life}");
	}
