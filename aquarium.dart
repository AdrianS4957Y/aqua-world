...
class Aquarium{
	...
	Aquarium({ double? height, double? width, double? length, int? fishesCount }){
		height = double.parse((height ?? math.Random().nextDouble()*50).toStringAsFixed(4));
		width = double.parse((width ?? math.Random().nextDouble()*50).toStringAsFixed(4));
		length = double.parse((length ?? math.Random().nextDouble()*50).toStringAsFixed(4));
		// sender = sender;
		final random = fishesCount ?? (math.pow(volume, 1/3)*10).floor();
		maxFishes = random < 10 ? 10 : random;
		print("У вас теперь есть аквариум c размерами ${height}*${width}*${length}. В нём могут поместится ${maxFishes} рыбок");
		initFishes();
		...
		timerReceiver.listen((message){
			...
			try{
				if(message["name"] == "time"){
					this.listPopulationTime.updateAll((id, time) => time+(message["value"] as int));
				fishList.forEach((Fish fish){
					...
					if(listOfBreedables[Gender.male]!.length < listOfBreedables[Gender.female]!.length){
						for(String id in listOfBreedables[Gender.male]!){
							if(fishList.length >= maxFishes)break;
							listPopulationTime.update(listOfBreedables[Gender.female]!.first, (i) => 0);
						...
						for(String id in listOfBreedables[Gender.female]!){
							if(fishList.length >= maxFishes)break;
							listPopulationTime.update(listOfBreedables[Gender.male]!.first, (i) => 0);
							listPopulationTime.update(id, (i) => 0);
							addFish();
							// listOfBreedables[Gender.female]!.remove(id);
							listOfBreedables[Gender.male]!.remove(listOfBreedables[Gender.male]!.first);
							}
						}
		...
		receiver.listen((message)async{
			...
			switch(message){
				case "RIP":
					...
					isolate.kill();
					receiver.close();
					await this.listPopulationTime.remove(fish!.id);
					break;
			}
		});
		// fishList.add(fish);
	...
	removeFish(Fish fish) async{
		fishList.removeWhere((Fish can) => can.id == fish.id);
	...
	String get status{
		...
		sorted.sort((a,b) => (b as MapEntry<String, int>).value - (a as MapEntry<String, int>).value);
		String str = "Кол-во рыбок ${fishList.length} ";
    ...
    int random = math.Random().nextInt(timeList["fishLife"] ~/ 2 * maxFishes ~/ (fishList.length == 0 ? 1 : fishList.length));
